...
function SearchIcon() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" aria-hidden>
      <path d="M21 21l-4.3-4.3M10.5 18a7.5 7.5 0 1 1 0-15 7.5 7.5 0 0 1 0 15z" stroke="#111" strokeWidth="1.8" strokeLinecap="round" />
    </svg>
  );
}
...
export type ContentItem = {
  id: string;
  type: ContentType;
  title: string;            // normalized, exactly 3 words
...
const asArray = (t?: string[] | string): string[] =>
  Array.isArray(t) ? t : t ? String(t).split(",").map(s => s.trim()).filter(Boolean) : [];
...
function cleanExcerpt({ raw, category, platform }: { raw?: string; category?: string; platform?: string }) {
...
export function normalizeItem(r: RawItem): ContentItem {
  ...
  const excerpt = cleanExcerpt({ raw: r.excerpt ?? r.description, category, platform });
  ...
  return {
    ...
    tags,
    cover: fixCoverUrl(r.thumbnail_url ?? r.image ?? r.cover, title),
...
function writeCounts(map: Record<string, number>) {
  try { localStorage.setItem(LS_COUNTS, JSON.stringify(map)); } catch {}
}
...
function writeVoted(map: Record<string, 1>) {
  try { localStorage.setItem(LS_VOTED, JSON.stringify(map)); } catch {}
}
...
function ContentCard({ item }: { item: ContentItem }) {
  ...
  const initials = creatorDisplay
    ? creatorDisplay.split(" ").map((s) => s[0]).join("").slice(0, 2).toUpperCase()
    : "HU";
  const date = new Date(item.dateISO).toLocaleDateString(undefined, {
  ...
  useEffect(() => {
    // hydrate from localStorage (per item)
    try {
        ...
        <div className="rounded-2xl border border-white bg-[#FFF5EA] p-4">
          ...
          <div className="flex items-center gap-3 text-sm">
            ...
            <div className="leading-tight">
              ...
              {item.creatorTitle && (
                <div className="text-neutral-600 text-[12px]">{item.creatorTitle}</div>
              )}
            </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-1">
                <MessageSquare className="w-5 h-5 text-neutral-600" strokeWidth={1.8} />
              ...
              >
                <ThumbsUp
                  className={`w-5 h-5 ${voted ? "text-[#2F80ED]" : "text-neutral-600"}`}
                  fill={voted ? "#2F80ED20" : "none"}
                  strokeWidth={1.8}
                />
                <span className={`text-[14px] font-medium ${voted ? "text-[#2F80ED]" : "text-neutral-700"}`}>
                  {likes}
                </span>
              </button>
            </div>
...
function FilterSidebar({
  ...
  return (
    <aside className="sticky top-20 h-fit rounded-2xl border border-black/10 bg-[#FDF9F0] p-5">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-[16px] font-semibold text-[#FF8A00]">Filters</h3>
        <button onClick={clearAll} className="text-sm text-neutral-600 hover:text-neutral-900">Clear all filters</button>
      </div>
...
export default function KnowledgeHubPage() {
  ...
  React.useEffect(() => {
    const API = process.env.NEXT_PUBLIC_KH_API || "/api/kh?source=csv&local=1&enrich=1";
  ...
  const filtered = useMemo(() => {
    ...
    const withinRange = (d: Date) => {
      ...
      if (dateRange === "This Week") { const past7 = new Date(now); past7.setDate(now.getDate() - 7); return d >= past7; }
      if (dateRange === "This Month") { const past30 = new Date(now); past30.setDate(now.getDate() - 30); return d >= past30; }
      return true;
    };

    let res = items.filter((it) => {
      const d = new Date(it.dateISO);
      const typeOk = selectedTypes.length ? selectedTypes.includes(it.type) : true;
      const creatorOk = selectedCreators.length ? selectedCreators.includes(it.creator) : true;
      const dateOk = withinRange(d);
      const tagOk = activeTag ? (it.tags.concat([it.title]).join(" ").toLowerCase().includes(activeTag.toLowerCase())) : true;
    ...
    if (sortBy === "date") res = [...res].sort((a, b) => +new Date(b.dateISO) - +new Date(a.dateISO));
    return res;
  }, [items, query, activeTag, selectedTypes, selectedCreators, dateRange, sortBy]);

  const recommended = useMemo(() => {
    const pool = activeTag
      ? items.filter((it) => (it.tags.join(" ").toLowerCase() + " " + it.title.toLowerCase()).includes(activeTag.toLowerCase()))
      : items;
    return [...pool].sort((a, b) => +new Date(b.dateISO) - +new Date(a.dateISO)).slice(0, 4);
  }, [items, activeTag]);
          ...
          <section>
            ...
            <div className="mt-6 flex items-center justify-between">
              <p className="text-sm text-neutral-700">
                Results for “{activeTag ? activeTag.toLowerCase() : query || "all"}”
              </p>
            ...
            <div className="mt-10 flex items-center justify-between">
              ...
              <Link href="#" className="text-sm text-neutral-600 hover:text-neutral-900">View All</Link>
            </div>
            <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-5">
              {recommended.map((item, i) => (
                <ContentCard key={`rec-${item.id}-${i}`} item={item} />
              ))}
            </div>
            ...
            <div className="mt-10 rounded-2xl border border-black/10 bg-[#FFF9F1] p-6">
              ...
              <div className="mt-4 flex items-center gap-3">
                <input className="flex-1 rounded-xl border border-black/10 bg-white px-4 py-3 text-[14px]" placeholder="Suggest a new fashion category or tag..." />
            ...
            <div className="flex flex-col sm:flex-row gap-16">
              ...
              <div>
                ...
                <ul className="mt-5 space-y-3 text-[14px]">
                  ...
                  <li><Link href="/mission" className="hover:text-white/90">Our Mission</Link></li>
                  <li><Link href="/knowledge-hub" className="hover:text-white/90">Knowledge Hub</Link></li>
                  <li><Link href="/luminaries" className="hover:text-white/90">Luminaries</Link></li>
                  <li><Link href="/faq" className="hover:text-white/90">FAQ</Link></li>
                  <li><Link href="/highlights" className="hover:text-white/90">Monthly Highlights</Link></li>
                  <li><Link href="/get-involved" className="hover:text-white/90">Get Involved</Link></li>
                  <li><Link href="/community" className="hover:text-white/90">Community Engagement</Link></li>
                </ul>
              </div>
